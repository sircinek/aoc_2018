defmodule Aoc2018.Day4 do
  ...
  def parse_input(input \\ input()) do
    ...
    |> Enum.map(
        fn({x,y}) ->
           ...
           {x_u, y |> String.trim() |> String.split()}
        end)
  end
  ...
  def b do
    ...
    |>(fn({id, {min,_count}}) -> parse_id(id) * min end).()

  end

  def parse_id(id), do: id |> String.trim("#") |> String.to_integer()

  def most_asleep_minute(updates) do
    Enum.reduce(0..59, {0,0},
    fn(i, {i_n, v}) ->
      count =
        Enum.reduce(updates, 0,
          fn({start, stop}, acc) ->
            if i in start.minute..stop.minute - 1 do
               acc + 1
            else
              acc
            end
  ...
  def group_shifts do
    ...
    |> Enum.map(
        fn({x,y}) ->
          ...
          u_x =
            case x.hour do
              23 ->
                case Calendar.ISO.days_in_month(x.year, x.month) do
                  ^d ->
                    %{x|hour: 0, minute: 0, day: 1, month: x.month + 1}
                  _ ->
                    %{x|hour: 0, minute: 0, day: x.day+1}
                end
              _ -> x
            end
          {u_x, y}
    ...
    |> Enum.reduce(%{},
         fn({_k, v}, acc) ->
          ...
          {_, n_acc} =
             Map.get_and_update(acc, id,
               fn(nil) ->
                   {nil, Enum.reverse(t)}
                 (time) ->
                   {time, time ++ Enum.reverse(t)}
               end
             )
         n_acc
         end
